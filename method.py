...
def get_imbalance_mask(sigmoid_logits, labels, nclass, threshold=0.7, imbalance_scale=-1):
    """
    If the prediction is correct, then the mask is 1. If the prediction is wrong, then the mask is 1. If
    the prediction is wrong, and the prediction is not the same as the label, then the mask is 1
    
    :param sigmoid_logits: the output of the model, before the sigmoid activation
    :param labels: the ground truth labels
    :param nclass: number of classes
    :param threshold: the threshold for the sigmoid function
    :param imbalance_scale: the weight of the negative samples
    :return: The mask and the multiclass_acc
    """
...
class OrthoHashLoss(BaseClassificationLoss):
    ...
    def compute_margin_logits(self, logits, labels):
        """
        If the margin type is cosine, then the margin logits are the cosine of the angle between the
        logits and the labels, otherwise the margin logits are the cosine of the angle between the
        logits and the labels plus the margin.
        
        :param logits: the output of the model before the softmax layer
        :param labels: the ground truth labels
        :return: The margin logits are being returned.
        """
        if self.m_type == 'cos':
            if self.multiclass:
...
def train(train_s_dataloader,
    ...
    for epoch in range(max_iter):
        ...
        for ((data_s, target_s, index), (data_t, target_t_gt, _)) in\
            ...
            total_loss = transfer_loss + loss_s + 0.1 * align_loss + 0.1 * loss_t
